...
object Search {
  ...
  def apply(gameVersionID: GameVersionID = GameVersionID.latestStable, sort: SortType = SortType.Name, startPage: Int = 1): Stream[Traversable[ModSearchResult]] = {
    ...
    Stream.range(startPage, numPages + 1).map {
      p =>
  ...
  def getAllSafeJ(gameVersionID: GameVersionID = GameVersionID.latestStable, sortType: SortType = SortType.Name, startPage: Int = 1, onUpdate: java.util.function.Consumer[java.util.List[ModSearchResult]]): java.util.List[ModSearchResult] = {
    apply(gameVersionID, sortType, startPage).foldLeft(Seq[ModSearchResult]())((acc, p) => {
